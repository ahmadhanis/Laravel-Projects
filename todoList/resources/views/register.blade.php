            ...
            <div class="w3-card w3-round">
                ...
                <div class="w3-padding ">
                    <form action="{{ route('register.post') }}" method="post" accept-charset="UTF-8">
                        ...
                        <p><input id="email" class="w3-input w3-round w3-border" type="name" name="name" required></p>
                        @if ($errors->has('name'))
                        ...
                        @if ($errors->has('email'))
                        <span class="w3-text-red">{{ $errors->first('email') }}</span><br>
                        @endif
                        <label for="pass">Password</label>
                        <p><input id="pass" class="w3-input w3-round w3-border" type="password" name="password" required></p>
                        @if ($errors->has('password'))
                        <span class="w3-text-red">{{ $errors->first('password') }}</span><br>
                        @endif
